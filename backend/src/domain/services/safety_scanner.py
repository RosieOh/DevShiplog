...
_PATTERNS: List[Tuple[str, Pattern[str]]] = [
    ...
    # --- 내부 리소스 ---------------------------------------------------------
    # 스킴이 붙은 내부 주소. http(s) 만 보면 안 된다.
    # 기술 글에는 redis:// postgres:// mysql:// mongodb:// amqp:// 가 그대로 붙여넣기 된다.
    (
        "internal_url",
        re.compile(
            r"[a-z][a-z0-9+.\-]*://"
            r"(?:[^\s/@]+@)?"  # user:pass@ 가 붙어 있어도 잡는다
            r"(?:localhost|127\.\d{1,3}\.\d{1,3}\.\d{1,3}|0\.0\.0\.0|"
    ...
    # 스킴 없이 적힌 사설 IP (예: "10.0.3.14:6379 로 붙었다").
    # localhost / 127.x 는 개발 글에 흔하고 유출 가치가 없어 스킴 있는 경우만 잡는다.
    (
        "internal_url",
        re.compile(
    ...
    (
        ...
        re.compile(
            ...
            r"\.(?:internal|local|corp|intranet|lan)(?::\d{1,5})?\S*",
            re.IGNORECASE,
        ),
    ),
]
