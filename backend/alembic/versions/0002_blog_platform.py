...
def downgrade() -> None:
    """
    autogenerate 가 만든 원본은 MariaDB 에서 동작하지 않았습니다.

    인덱스를 테이블보다 먼저 지우려 하는데, 그 인덱스의 첫 컬럼이 외래키라
    "외래키에 필요한 인덱스" 라며 거절합니다 (error 1553).
    그리고 MySQL 은 DDL 을 롤백하지 않으므로, 중간에 실패하면 스키마가
    반쯤 지워진 채로 남습니다 — 롤백을 시도하지 않느니만 못한 상태입니다.

    인덱스는 따로 지우지 않습니다. drop_table 과 drop_column 이 같이 지웁니다.
    """
    # 자식 → 부모 순서. 외래키가 걸린 쪽을 먼저 지운다.
    ...
    # users 는 0001 에서 만든 테이블이라 컬럼만 되돌린다.
    # handle 의 유니크 인덱스는 컬럼과 함께 사라진다.
