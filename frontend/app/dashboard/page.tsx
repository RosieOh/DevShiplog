    ...
    <div className="bg-canvas min-h-screen">
      <div className="max-w-[1400px] mx-auto px-[5%] py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          <Link
            href="/drafts/new"
            className="bg-surface p-8 rounded-[32px] border border-black/5 motion-safe:hover:-translate-y-2 transition-transform"
          >
        ...
        <div className="bg-surface rounded-[32px] border border-black/5 p-10">
          ...
          {loading ? (
            <div className="text-center py-16">
              <div className="motion-safe:animate-spin motion-reduce:animate-pulse rounded-full h-12 w-12 border-2 border-line border-t-ink mx-auto"></div>
            </div>
          ) : drafts.length === 0 ? (
            <div className="text-center py-16">
              ...
              <Link
                href="/drafts/new"
                className="inline-block px-8 py-4 bg-accent text-ink rounded-full font-semibold motion-safe:hover:scale-105 transition-transform"
              >
                첫 Draft 만들기
              </Link>
            </div>
            ...
            <div className="space-y-4">
              {drafts.map((draft) => (
                <Link
                  ...
                  className="block p-6 border border-black/5 rounded-2xl motion-safe:hover:-translate-y-1 transition-transform"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-4 py-2 rounded-full text-xs font-semibold ${
                      draft.status === 'active' ? 'bg-accent text-ink' : 'bg-gray-100 text-ink-muted'
                    }`}>
                      {draft.status}
                    </span>
                  </div>
